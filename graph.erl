...
-export([from_file/1, from_file/3, del_graph/1, vertices/1, edges/1, edge_weight/2,
        ...
        merge_list/2,communicate/3, extract/1, my_color/2,
        handle_mess/0, update_color/5, update_target/5, assign_target/2, my_target/2,
...
handle_call({write, FileName, OldList, PidF, PidP}, _From, State) ->
  ...
  Updated1 = update_color(TargetColor,PidP,OldList,OldList,0),% aggiorno il colore del padre con il suo target
  Updated2 = update_target(TargetColor,PidF,Updated1,Updated1,0),% aggiorno il colore target dell'adiacente con il target del padre
...
update_color(_, _, [], NodesL, _) -> NodesL;
update_color(NewColor,Pid,[{IdNodo,_,S,Tar} | T], NodesL, Count) ->
    Pid2 = whereis(list_to_atom(integer_to_list(IdNodo))),
      case Pid of     
        Pid2 -> 
              NewTuple = {IdNodo, NewColor, S, Tar},
              UpdatedNodesL = lists:sublist(NodesL,Count) ++ [NewTuple] ++ lists:nthtail(Count+1,NodesL),
              update_color(NewColor,Pid, T ,UpdatedNodesL, Count);
          _ -> 
              update_color(NewColor,Pid,T, NodesL, Count+1) % Altrimenti continua a cercare nella lista
      end.
  
%% aggiorna il target di Pid
update_target(_, _, [], NodesL, _) -> NodesL;
update_target(NewTarget,Pid,[{IdNodo,C,S,_} | T], NodesL, Count) ->
    Pid2 = whereis(list_to_atom(integer_to_list(IdNodo))),
      case Pid of     
        Pid2 -> 
              NewTuple = {IdNodo, C, S, NewTarget},
              UpdatedNodesL = lists:sublist(NodesL,Count) ++ [NewTuple] ++ lists:nthtail(Count+1,NodesL),
              update_target(NewTarget,Pid, T , UpdatedNodesL, Count);
          _ -> 
              update_target(NewTarget,Pid, T, NodesL, Count+1) % Altrimenti continua a cercare nella lista
      end.
